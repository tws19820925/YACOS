...
- `privacy_policy.html`: アプリ紹介、タロット/おみくじ画像一覧、安全な使い方、プライバシーポリシー本文
...
- `assets/ritual/tarot/*.webp`: タロットカード一覧に掲載する縮小プレビュー画像
- `assets/ritual/omikuji/*.webp`: おみくじ一覧に掲載する縮小プレビュー画像
- `assets/ritual/gallery_manifest.json`: ギャラリー画像の件数、寸法、HTML透かし方針
- `scripts/build_ritual_gallery_assets.py`: Androidアプリ側の儀式画像から公開ページ用プレビューを再生成するスクリプト
...
## Ritual Gallery Update Checklist

- [ ] `D:\GitHub\yacos_fortune_telling\app\src\main\res\drawable-nodpi` のタロット/おみくじWebPが最新である。
- [ ] `python scripts\build_ritual_gallery_assets.py --updated-at YYYY-MM-DD` で公開用プレビューを再生成する。
- [ ] 公開ページ上ではCSS疑似要素で `©arcadia-labs` の透かしを重ねる。画像ファイルへ透かしを焼き込まない。
- [ ] タロット78枚、おみくじ7種の件数が `assets/ritual/gallery_manifest.json` と一致する。
- [ ] `powershell -ExecutionPolicy Bypass -File scripts/verify_public_assets.ps1` が成功する。
